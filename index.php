...
sync_recovered_seo_premium_content_20260723();
...
function sync_recovered_seo_premium_content_20260723(): void {
    $file = __DIR__ . '/data-recovered-seo-premium-content.php';
    if (!is_file($file)) return;
    $items = require $file;
    if (!is_array($items)) return;
    $checksum = md5_file($file) ?: '';
    try {
        if (function_exists('setting') && setting('recovered_seo_premium_content_checksum_20260723', '') === $checksum) return;
    } catch (Throwable $e) {}
    foreach ($items as $item) {
        if (empty($item['slug']) || empty($item['title'])) continue;
        $exists = Database::one("SELECT id FROM posts WHERE slug=? LIMIT 1", [$item['slug']]);
    ...
    try {
        if (function_exists('save_setting')) save_setting('recovered_seo_premium_content_checksum_20260723', $checksum);
    } catch (Throwable $e) {}
}
